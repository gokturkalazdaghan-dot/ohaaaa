-- ============================================================================
-- YAPAY ZEKÂ BÜTÇESİ — tavan gerçekten durduruyor mu?
-- ----------------------------------------------------------------------------
...
begin
  ...
  ---------------------------------------------------------------------------
  for i in 1 .. 3 loop
    s := public.consume_ai_budget('test:kova:a', 3, 3600);
  ...
  ---------------------------------------------------------------------------
  s := public.consume_ai_budget('test:kova:b', 3, 3600);
  if (s ->> 'allowed')::boolean is not true then
  ...
  ---------------------------------------------------------------------------
  update public.ai_rate_counters
  ...
  ---------------------------------------------------------------------------
  s := public.consume_ai_budget('test:kova:c', 1, 3600);
...
do $$
begin
  if has_table_privilege('anon', 'public.ai_rate_counters', 'SELECT') then
    raise exception 'BAŞARISIZ: anon sayaci okuyabiliyor';
  end if;
  if has_table_privilege('authenticated', 'public.ai_rate_counters', 'UPDATE') then
    raise exception 'BAŞARISIZ: authenticated sayaci degistirebiliyor';
  end if;
  if has_function_privilege('anon', 'public.consume_ai_budget(text, integer, integer)', 'execute') then
    raise exception 'BAŞARISIZ: anon sayaci kendi artirabiliyor';
  end if;
  if not has_function_privilege('service_role', 'public.consume_ai_budget(text, integer, integer)', 'execute') then
