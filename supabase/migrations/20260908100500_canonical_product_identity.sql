...
--
-- Eksik olan, iki ayrı feed'in AYNI ürünü gönderdiğinde onların AYNI
-- `product_groups` satırına düşmesini sağlayan DETERMİNİSTİK anahtar. O
-- anahtar olmadan `group_id` elle doldurulan bir alan olarak kalır ve
-- pratikte hiç dolmaz: katalog, aynı telefonun 400 ayrı "ürün" olarak
-- göründüğü bir listeye döner.
--
-- ---------------------------------------------------------------------------
-- ANAHTAR VERİTABANINDA, KODDA DEĞİL
-- ---------------------------------------------------------------------------
-- Kod tarafında "önce SELECT, eşleşen grup var mı, yoksa INSERT" yapılabilirdi
-- ama yüz milyonluk hacimde o kalıp tam olarak kaçındığımız yarışı üretir:
-- iki alım işçisi aynı anda "yok" görür, ikisi de yazar, kanonik ürün ikiye
-- bölünür. `canonical_key` ÜRETİLMİŞ (generated) ve TEKİL.
--
-- ---------------------------------------------------------------------------
...
-- AYNI ürünü gösterir. Ham metin karşılaştırması bunları üç ayrı ürün sayar
-- -- tekilleştirmenin en sık sessizce kaçırdığı durum budur.
-- ===========================================================================
...
/**
 * Boşlukları tek gösterime indirir.
 *
 * ŞART, çünkü feed'ler aynı başlığı farklı boşluklarla gönderir:
 * "Urun  cok   bosluklu" ile "Urun cok bosluklu" AYNI üründür. Boşluk farkı
 * anahtara girseydi, aynı ürün her feed'de yeni bir kanonik satır açardı.
 */
...
/*
 * TEKİLLİK BURADA KURULUYOR. Bu indeks olmadan `on conflict (canonical_key)`
 * yazılamaz ve alım hattı SELECT-then-INSERT'e, yani yarışa mecbur kalır.
 */
...
-- `products.group_id` nullable KALIYOR: bağlanamamış bir teklif KAYBEDİLMEZ,
-- yalnızca karşılaştırmaya girmez. NOT NULL yapmak, eşleşmeyen tek bir feed
-- satırının tüm partiyi düşürmesi demekti.
